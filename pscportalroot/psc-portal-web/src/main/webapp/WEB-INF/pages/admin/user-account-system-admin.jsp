...
<div class="breadCrumb">User Accounts</div>
<h1>User Accounts</h1>
		...
		<div class="tabR">
			<div class="tabM">
                ...
                <c:when test='${role eq "Provider" }'>active"</c:when>
					<c:otherwise>" href="<c:url
							value='/system/user/search?role=Provider' /></c:otherwise> </c:choose> <span class="aR">
							<span class="aM">Providers</span></span></a> 
						<a class="tabLink <c:choose>
                <c:when test='${role eq "Service Agent" }'>active"</c:when>
					<c:otherwise>" href="<c:url
							value='/system/user/search?role=Service Agent' />"</c:otherwise> </c:choose> ><span
					class="aR"><span class="aM">Service Agents</span></span></a> 
					<a class="tabLink <c:choose>
                <c:when test='${role eq "Service Administrator" }'>active"</c:when>
					<c:otherwise>" href="<c:url value='/system/user/search?role=Service Administrator' />"</c:otherwise> </c:choose> ><span
					class="aR"><span class="aM">Service Administrators</span></span></a> 
					<a class="tabLink <c:choose>
                <c:when test='${role eq "System Administrator" }'>active</c:when>
					<c:otherwise>" href="<c:url
							value='/system/user/search?role=System Administrator' />"</c:otherwise> </c:choose> ><span
					class="aR"><span class="aM">System Administrators</span></span></a> <a
					class="purpleBtn"
					href="<c:url value='/system/user/new?role=${role}' />"><span
					class="btR"><span class="btM">Create New User Account</span></span></a>
			</div>
		</div>
	</div>
	<!-- /.tabHead -->
...
<input type="hidden" value="<c:url value='/system/user/delete' />"
	id="deleteAccountsURL" />
